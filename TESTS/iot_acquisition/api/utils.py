...
class CarsHelper:
    ...
    def get_cars(self, request):
      ...
      str_query = 'SELECT * FROM ' + self.gdcb.config_data["CARSXCODESV2_TABLE"] + ' as T WHERE EXISTS (SELECT * FROM (VALUES ' + ','.join(map(str, tuples)) + ') AS V(CarID, Code) WHERE T.CarID = V.CarID and T.Code = V.Code)'
